...
-   **Unreviewed Item Counts on Mentor Dashboard**: Mentor dashboard now shows per-client amber badges with counts of unreviewed check-ins, reflections, and exercises (via GET /api/therapist/unreviewed-items). Batch-optimized queries avoid N+1 performance issues. Cache invalidation added: badges refresh immediately when feedback is submitted or items are marked reviewed. Pending Reviews card removed from mentor dashboard (redundant with per-client badges).
-   **Client Analytics Trend Rules**: Analytics page (`analytics.tsx`) uses research-based `computeTrend()` matching server-side `trendAnalysis.ts` semantics: increasing/decreasing/stable/consistently_same/insufficient_data. MIN_ENTRIES=5, MCID_THRESHOLD=2.0. Trend badges and insight messages only show when clinically meaningful. Insight text adapts for client vs mentor/admin viewers.
