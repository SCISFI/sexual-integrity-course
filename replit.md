...
-   **Admin Tools**: User management (create/edit/delete mentors and clients), fee waivers, client deletion, revenue tracking, editing of any feedback. Admins can edit mentor name, email, and password via the Mentors tab Edit button (PATCH `/api/admin/therapists/:id` accepts `name`, `email`, `newPassword`). **Cohorts**: Admins can group clients into cohorts (Cohorts tab), manage membership, and view per-cohort analytics and cohort comparison charts via `/admin/cohorts/:id` and `/admin/cohorts/:id/analytics`.
