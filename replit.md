...
-   **Database Schema**: Core tables include `users`, `week_completions`, `daily_checkins`, `week_reflections`, `homework_completions`, `therapist_clients`, `therapist_feedback` (extended with `status`, `sent_at`, `subject`), `feedback_replies`, `week_fee_waivers`, `password_reset_tokens`, `dismissed_guidance_suggestions`, `parent_consent_tokens`, `parent_client_relationships`, and `parent_messages`.
...
-   **User Roles**: Admin (full access), Mentor (client progress, feedback), Client (curriculum, daily check-ins, homework, payments), Parent (read-only teen progress + mentor messaging).
-   **Adolescent Program Track**: Teens (ages 13–17 / still in high school) register via `/register/adolescent`. Account is created with `subscriptionStatus: 'pending_consent'` and `programType: 'adolescent'`. A consent token email is sent to the parent/guardian. Parents approve at `/parent-consent/:token`, creating a `parent` role account and activating the teen's account. Parents see progress (weeks completed, streak, last active) at `/parent-dashboard` and can message the mentor. Mentors see a "Teen" amber badge on adolescent clients and can view/reply to parent messages from the client detail page. Admin can create adolescent clients with the same consent flow from the "Add Client" dialog. Adolescent curriculum lives in `client/src/data/adolescent-curriculum.ts` (16 placeholder weeks with `WeekContent` shape — ready for real content). Week pages auto-select the right curriculum based on `user.programType`.
...
-   **Mentor Message System**: Terminology: "Message" (not "feedback") in all mentor/client UI. `therapist_feedback` table stores messages with status (draft/sent), sentAt, and subject fields. Guidance-tab-originated messages use `feedbackType='guidance'` and trigger the branded `sendMentorMessage()` email (full body in email, navy/slate design). Clients only see `status='sent'` messages. `dismissed_guidance_suggestions` table allows mentors to dismiss non-urgent suggestions per client. Draft messages persist and are surfaced in the Guidance tab draft inbox. **Client Replies**: Clients can reply to sent mentor messages from their dashboard via the `feedback_replies` table. Each message has a "Reply" toggle that expands a textarea + send button; replies appear chronologically with "You" labels. Mentors see unread reply count badges (red) on feedback items in the client detail page, and can expand to view replies (marked as read on fetch). Endpoints: `POST /api/messages/:feedbackId/reply` (client), `GET /api/messages/:feedbackId/replies` (client+mentor, marks read for mentor), `GET /api/therapist/unread-replies` (mentor/admin aggregate counts).
